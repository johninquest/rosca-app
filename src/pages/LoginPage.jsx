...
export default function LoginPage() {
  ...
  // Fix 1: While auth resolves, show spinner instead of flashing the login UI
  if (user === undefined) {
    return (
    ...
    <div className="min-h-dvh flex flex-col items-center justify-center bg-[#F9F9F9] px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white border border-[#E0E0E0] rounded-lg p-8 shadow-sm">
          {/* Fix 2: Disabled + spinner + label change while signing in */}
          <button
            type="button"
          ...
          {error && (
            <p className="mt-3 text-sm text-center text-[#1A1A1A]" role="alert">{error}</p>
          )}
        </div>
      </div>
    </div>
